...
// Shared directory-tree service. Each plugin instantiates DirectoryService<ItsContext>
// in DI and registers an IDirectoryRootRegistry implementation that surfaces
// the plugin's WellKnownDirectoryIds.
public class DirectoryService<TContext> : ServiceBase<TContext, Directory>, IDirectoryService
    where TContext : SharedDbContext
{
    private readonly IDirectoryRootRegistry _roots;

    public DirectoryService(TContext db, IUserService userService, IDirectoryRootRegistry roots)
        : base(db, userService)
    {
        _roots = roots;
    }
    ...
    public async Task<Directory> ChangeParent(Guid id, Guid newParentId)
    {
        if (id == newParentId)
        {
            throw new EdmException("Edm.Folder.OwnParent", "Folder cannot be its own parent.");
        }

        if (id == Directory.GeneralRootId || _roots.IsTypeRoot(id))
        {
            throw new EdmException("Edm.Folder.BuiltInNotMovable", "Built-in folders cannot be moved.");
        }

        if (newParentId == Directory.GeneralRootId)
        {
            throw new EdmException(
                "Edm.Folder.MustLiveInTypeRoot",
    ...
    public async Task<Guid?> ResolveTypeRoot(Guid folderId)
    {
        ...
        {
            if (_roots.IsTypeRoot(folder.Id))
            {
                return folder.Id;
            }
