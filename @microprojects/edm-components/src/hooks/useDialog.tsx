...
type StoredProps = Omit<DialogProps, 'close'>;
type SetterProps = Omit<StoredProps, 'destructive'>;

export const useDialog = () => {
    const [props, setProps] = React.useState<StoredProps | null>(null);
    const close = () => setProps(null);
    const dialog = props ? <Dialog {...props} close={close} /> : null;

    const confirm = (p: SetterProps) => setProps({destructive: false, ...p});
    const warning = (p: SetterProps) => setProps({destructive: true, ...p});

    return {dialog, confirm, warning};
};

function Dialog({destructive, title, message, actionLabel, onConfirm, close}: DialogProps) {
    const handleConfirm = () => {
        onConfirm && onConfirm();
        close();
    };
