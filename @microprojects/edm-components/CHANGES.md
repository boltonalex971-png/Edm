...
- **`RelationTable` — `toolbarStart` / `toolbarEnd` slots.** Two new optional ReactNode props that inject content into the existing toolbar row: `toolbarStart` renders before the search field (left side), `toolbarEnd` after the spacer and before any "Add record" button (right side). Call sites can park inline controls (filter switches, mode toggles, auxiliary actions) on the same row as Search and primary add instead of stacking a separate Box above the grid. Logistics's Order Specification tab uses `toolbarEnd` for its "Hide totally allocated components" switch.
- **`RelationTable` — `readonly` prop and stricter per-flag semantics.** New `readonly?: boolean` prop (default `false`) is a one-shot disabler that overrides `creatable`/`editable`/`removable` to false — same name and intent as `Detail`'s `readonly`, suitable for view-only tables (Logistics's Order Specification and Operation tabs adopt it). The three per-flag props now actually hide their UI: `editable={false}` hides the per-row Edit button (was: button always visible), `removable={false}` hides the per-row Delete button (was: button visible-but-disabled), and `creatable` defaults to plain `true` instead of being implicitly tied to `editable !== false` so the three concerns are independent. When neither editable nor removable remains, the actions column is dropped entirely so the table reads as pure data instead of reserving 96 px of empty space.
