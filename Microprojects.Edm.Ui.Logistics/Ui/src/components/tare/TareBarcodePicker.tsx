...
export const TareBarcodePicker = ({
    ...
    useEffect(() => {
        let cancelled = false
        const url = `${api.tares}/available?tareTypeId=${tareTypeId ?? ''}&nomenclatureId=${nomenclatureId ?? ''}&barcode=${barcode ?? ''}&includeFull=${includeFull}`;
        (async () => {
            try {
    ...
    }, [tareTypeId, nomenclatureId, barcode, includeFull])
