...
export function SmartScrollContent({
    children,
    flex,
    style,
}: SmartScrollContentProps) {
    return (
...
function smartScroll(marginTop: number) {
    ...
    contents = Array.from(
        viewport.getElementsByClassName('smart-scroll-content'),
    ...
    function scrolled() {
        ...
        for (const element of contents) {
            ...
            if (container.clientHeight <= element.clientHeight) {
                ...
                if (currPos < windowScrollHeight) {
                    // down
                    state = 'down'
                    style = {
                        position: 'sticky',
                        top: `-${element.clientHeight - windowHeight + marginTop}px`,
                    }
                } else {
                    // up or stay
                    state = 'up'
