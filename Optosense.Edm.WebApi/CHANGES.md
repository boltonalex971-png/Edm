...
- **Installation packaging fixes** (PR #16). `Build-EdmSetup.ps1` passes `/p:FileVersion` only (never `/p:AssemblyVersion`, which cascades and breaks plugin loading), runs the MSI patch step out-of-process so SummaryInfo COM doesn't fail with `DISP_E_TYPEMISMATCH` after `devenv`, and writes SummaryInfo via `PutDispProperty + Persist`.
