...
- **Platform restructure — new plugin contracts, retired Host & Shared assemblies** (PR #46). Plugin platform code is lifted out of `Microprojects.Edm.Host` and `Microprojects.Edm.Shared` (both projects retired) into the host project `Optosense.Edm.WebApi` and the shared libraries `Optosense.Edm.Core`, `Optosense.Edm.Core.AspNet`, `Optosense.Edm.Domain` and `Optosense.Edm.Infrastructure`. Plugin contracts (`IPlugin`, `IDriverPlugin`, `IOperationPlugin`, `IProfilePlugin`, `IApplicationPlugin`, `PluginManager`, `PluginRegistry`, plugin attributes and bases) now live under `Edm/Plugins/*`. Out-of-tree plugins and tooling need to update their `using` directives and project references; see `CLAUDE.md` for the current layout.
- **Plugin discovery by folder** (PR #51). The config moves from `Edm:Assemblies` (explicit DLL paths) to `Edm:PluginsPaths` (folder list, defaulting to `./Plugins`). The host scans each listed folder top-level and one subdirectory deep for any `[PluginAttribute]`-decorated class. Update overrides in `appsettings.Development.json` and any deployment overlays — the old key is no longer read.
- **Hub application plugin ships in the MSI** (PR #53). New `Microprojects.Edm.Ui.Hub` application plugin provides the cross-plugin launcher / home page; every existing application plugin (Technologies, Console, Logistics) now ships an `ABOUT.md` resource used by the Hub's About panel and is listed automatically in the launcher.
- **Shared frontend components package** (PR #56, PR #57, PR #58, PR #55, PR #54). A new `@microprojects/edm-components` npm package extracts the visual primitives (Field, EditorSection, Properties, Detail/MasterDetail, TreeViewMaster, HierarchyPicker, RelationTable, Changelog, AuthInterstitial, ValueFlash, density/scheme controls, plus hooks for entity-locks, refresh, dialogs, get/post, SignalR) and the design tokens used across Technologies, Console, Logistics and the new Hub. Plugins must import the package's `tokens.css` or surface variables won't resolve.
- **Optional Negotiate authentication** (PR #49). Windows Negotiate can now be turned off via `Edm:Auth:Negotiate:Enabled` (`bool?`); when unset, the host defaults to Negotiate on Windows and JWT-only off-Windows. Non-domain hosts, headless integration test runs and Linux dev boxes can boot the host without forcing Kerberos / NTLM. The spec in `docs/specs/auth-implementation-spec.md` covers the new fallback chain.
- **Plugin lifecycle hardening** (PR #48). The host's `PluginLifecycleService` shutdown path tolerates plugins whose `Shutdown` throws, so a misbehaving plugin no longer aborts service shutdown or leaves the remaining plugins in an undefined state. Anonymous-auth handler in the integration tests was extracted alongside, but production-side this is purely a stability fix.
- **AutoMapper removed** (PR #52). `Microprojects.Edm.Shared.Mapper.*` and all `_*WebModelsProfile` / `_InfrastructureModelsProfile` types are gone; controllers, services and view-model mappings (`WebModelMappings`, `InfrastructureModelMappings`) are explicit code. One fewer reflection-driven dependency and no more startup-log mapping warnings.

### Operational notes

- **Upgrade path**: v2.0.0 is `RemovePreviousVersions=TRUE` over v1.x, but the major-version jump means a fresh `ProductCode`/`PackageCode` is required in the vdproj — rotate both when stamping the build, otherwise the upgrade is treated as a downgrade by Windows Installer.
- **Config migrations**: rename `Edm:Assemblies` → `Edm:PluginsPaths` in any host config overlay; add `Edm:Auth:Negotiate:Enabled` if you need to disable Negotiate on a Windows host.
- **Plugin code**: out-of-tree plugins need to reference the new `Edm/Plugins/*` namespace and switch from the old `Microprojects.Edm.Host` / `Microprojects.Edm.Shared` references to the new shared libraries.
