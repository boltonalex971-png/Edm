...
export const Options = ({ data }) => {
    ...
    if (data && !endpoint && !output) {
        const outputs = (data.output || [])
            .map(el => ({ text: el, value: data.options?.output?.find(p => p.text === el)?.value || null }))
        setOutput(outputs)
        setEndpoint(data.options?.endpoint)
    }
    const handleSubmit = (o) => {
        axios.put(`${data.api}`, { ...o, output });
    };
                                ...
                                <Field name={'output'}
                                    component={(fieldProps) =>
                                        <MultiSelect {...fieldProps}
                                            data={data.output.filter(o => !output?.some(a => a.text === o)).map(el => ({ text: el }))}
                                            onChange={(e) => {
                                                setOutput(e.value);
