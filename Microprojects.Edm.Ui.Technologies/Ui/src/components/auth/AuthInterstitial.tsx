...
const BODY_COPY: Record<InterstitialKind, (user?: string) => string> = {
    signin: () => 'You are not authenticated to access the EDM Technologies application. Please sign in with your domain account.',
    expired: () => 'You have been signed out for inactivity. Sign in again to continue where you left off.',
    'no-role': (user) => `${user ? `User ${user} has no role assigned in EDM. ` : 'No role is assigned to your account. '}Please contact your system administrator to request access.`,
...
export function AuthInterstitial({ kind, user, onAction }: AuthInterstitialProps) {
