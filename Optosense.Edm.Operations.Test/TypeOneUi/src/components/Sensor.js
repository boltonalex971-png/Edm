export const Sensor = ({ info, settings, addr}) => {
    ...
    return (
        <div
            ...
            {settings?.indicators && settings.indicators.sort((a, b) => a.order - b.order)
                .map(i => 
                    <span
                        key={`${addr}${i.indicator}`}
                        className={(info[i.parameter]?.valid !== undefined && `bg-${(info[i.parameter].valid && 'success') || 'danger'} text-light`) || ''}
