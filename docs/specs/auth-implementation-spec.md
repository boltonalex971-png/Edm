...
Single source of truth for how authentication and authorization are wired in `Optosense.Edm.WebApi`. Read this before changing anything under `Optosense.Edm.WebApi/Extensions/EdmAuthExtensions.cs` (auth pipeline + JWT cookie refresh), `Optosense.Edm.Core.AspNet/Auth/**`, `Optosense.Edm.Core.AspNet/SignalR/**`, `Optosense.Edm.Core/Infrastructure/IClientCertificateProvider.cs`, `Optosense.Edm.Infrastructure/Edm/Jobs/GrpcJobExecutor.cs`, `Optosense.Edm.Infrastructure/Edm/RemoteJobs.cs`, or any `appsettings*.json` `Edm:Auth` / `Edm:Intercom` / `Kestrel:Endpoints` section. The composition root in `Optosense.Edm.WebApi/Program.cs` only invokes `builder.AddEdm()` / `app.UseEdm()` — auth lives in `EdmAuthExtensions`, called from `EdmHostBuilderExtensions.AddEdm`.
...
`EdmAuthExtensions.AddEdmAuth` registers a `PolicyScheme` named `SmartAuth` that forwards by request shape and port:
...
- **Private CA → `RevocationMode = NoCheck` is mandatory.** The default `Online` setting hits CRL/OCSP URLs that private CAs don't publish; the handler then rejects every connection with `RevocationStatusUnknown`. The setting is enforced in `EdmAuthExtensions.ConfigureCertificate` — don't remove it.
